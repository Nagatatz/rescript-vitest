...
コードの変更を伴う作業は **`main` から新しいブランチを作成して行うこと**（命名は下表）。マージ前に `tasklist.md` の全タスク（マージタスク自体を含む）が `[x]` で、その更新がマージ前の最終コミットに含まれていることを確認する。

> **ブランチ作成 → コミット → PR → マージの実行手順は `git-workflow` skill が SSOT。**
